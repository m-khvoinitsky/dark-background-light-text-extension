...
if (!available) {
    ...
    let toast_duration;
    try {
        show_toast = NativeWindow.toast.show;
        toast_duration = {
            short: 'short',
            long: 'long'
        };
    } catch (e) {
        // at least in fennec 44 Snackbars.jsm is not working. so, using it only if NativeWindow.toast fails.
    ...
    exports.AndroidNativeWindow = Class({
        ...
        unload: function unload() {
            ...
            PageActions.remove(self.page_action_id);
        }
    })
}
