...
async function get_method_for_url(url) {
    ...
    if (prefs.enabled) {
        if (is_iframe) {
            if (await browser.runtime.sendMessage({action: 'does_parent_affects_me'})) {
                return methods[0];
            }
        }
...
browser.runtime.onMessage.addListener(async (message, sender) => {
    try {
        ...
        switch (message.action) {
            case 'do_you_affect_your_children':
                return (await current_method_promise).affects_iframes;
            default:
