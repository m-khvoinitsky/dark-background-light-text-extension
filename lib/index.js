...
const isSeaMonkey = require('sdk/system/xul-app').ID === '{92650c4d-4b8e-4d2a-b7eb-24ecf4f6b63a}';

const { getMostRecentBrowserWindow } = require('sdk/window/utils');
function get_current_tab_nsIDOMWindow() {
    let aDOMWindow = getMostRecentBrowserWindow();
    if (
        aDOMWindow &&
        aDOMWindow.gBrowser &&
        aDOMWindow.gBrowser.selectedBrowser &&
        aDOMWindow.gBrowser.selectedBrowser.contentWindow
    )
        return aDOMWindow.gBrowser.selectedBrowser.contentWindow;
}
...
const built_in_configured = {
    ...
    //'about:addons': '4'
    // SeaMonkey
    'chrome://navigator/content/navigator.xul': '0',
    'chrome://messenger/content/messenger.xul': '0',
    'chrome://messenger/content/AccountWizard.xul': '0',
    'chrome://editor/content/editor.xul': '0',
    'chrome://messenger/content/addressbook/addressbook.xul"': '0',
    'chrome://global/content/commonDialog.xul': '0',
};
...
function generate_urls(url_param) {
    let url_str;
    if (!url_param)
        url_str = isSeaMonkey ? get_current_tab_nsIDOMWindow().document.documentURI : tabs.activeTab.url;
    else
        url_str = url_param;
...
function get_settings_init_params() {
    return {
        ...
        isPrivate: isSeaMonkey ? (isPrivate(get_current_tab_nsIDOMWindow())) : (isFennec ? isPrivate(getTargetWindow(tabs.activeTab)) : isPrivate(tabs.activeTab)),
