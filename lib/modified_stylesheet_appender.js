...
var color_utils = require('./color_utils.js');
...
var process_background_image = function(bg_image, bg_repeat, selector, base_url) {
    if (bg_image.indexOf('url(') === 0) {
        ...
        let rel_url = bg_image.slice(bg_image.indexOf('("') + 2, bg_image.lastIndexOf('")'));
        if (rel_url.indexOf('data:') != 0) {
            let url = rel_url_to_abs(rel_url, base_url);
