...
export function is_IPv4(maybe_ip: string): boolean {
  ...
  for (let i = 0; i <= maybe_ip.length; i++) {
    ...
    if (
      ...
      if (
        number_of_octets > 4
        || (number_of_octets < 4 && i === maybe_ip.length)
      ) {
        return false;
      }
      if (first === null) {
...
export function generate_urls(
  ...
  try {
    ...
    const protocol_real = url_obj.href.startsWith(`${url_obj.protocol}//`)
      ...
      : url_obj.protocol;
    const is_http =
      url_obj.protocol === 'http:' || url_obj.protocol === 'https:';
    ...
    if (!is_IPv4(url_obj.hostname) && url_obj.hostname.indexOf('.') >= 0) {
      ...
      if (subdomain_parts) {
        for (let i = 0; i < subdomain_parts.length; i++) {
          result_list.push(
            `${subdomain_parts
              .slice(i, subdomain_parts.length)
              .join('.')}.${domain}`,
          );
        }
      }
      ...
      result_list.push(
        is_http ? url_obj.hostname : `${protocol_real}${url_obj.hostname}`,
      );
    }
    ...
    if (result_list.length === 0) {
      console.error(
        `generate_urls: no urls has been generated, returning original: ${url_str}`,
      );
    ...
    console.error(
      `generate_urls: something went wrong, returning original URL: ${url_str}`,
      e,
    );
