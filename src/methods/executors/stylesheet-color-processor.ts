...
function parse_text_shadow(
  value: string,
): [null, null] | [Array<string | RGBA>, number] {
  let color_index;
  ...
  implements MethodExecutor
{
  ...
  readonly rename_var_fg = (var_name: string) =>
  ...
  constructor(window: Window, options: AddonOptions) {
    ...
    const default_foreground_color = parseCSSColor(
      this.options.default_foreground_color,
    )!;
    const default_background_color = parseCSSColor(
    ...
    const default_colors: DefaultColors = is_dark_background
      ? {
  ...
  all_sheets_have_been_processed() {
    // TODO: remove hardcoded "simple-css"
  ...
  process_background_image(
    ...
    if (bg_image.indexOf('url(') === 0) {
      ...
      let url = bg_image.slice(
        bg_image.indexOf('(') + 1,
        bg_image.lastIndexOf(')'),
      );
      url = url.trim();
      ...
      if (intersect(remove_background_image, [url, selector])) {
        ...
        // no-repeat combined with exact bg position is most likely sprite
