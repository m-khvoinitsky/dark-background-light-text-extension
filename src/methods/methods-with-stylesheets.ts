import { methods as methods_bare } from './methods';
import type { MethodsMetadataWithStylesheets } from '../common/types';
...
for (const key of Object.keys(methods_bare)) {
  for (let i = 0; i < methods_bare[key].stylesheets.length; i++) {
    switch (methods_bare[key].stylesheets[i].name) {
      ...
      default:
        throw Error(
          `Unknown stylesheet name: ${methods_bare[key].stylesheets[i].name}`,
        );
    }
  }
}
