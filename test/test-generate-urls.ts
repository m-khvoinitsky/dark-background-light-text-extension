...
import {
  ...
  hint_marker,
  is_IPv4,
} from '../src/common/generate-urls';
import { smart_generate_urls } from '../src/common/smart-generate-urls';

const DUMB_DOMAIN_DETECTION = '__DUMB__';
const test_urls = {
  'about:config': [hint_marker, 'about:config', 'about:'],
  'about:preferences': [hint_marker, 'about:preferences', 'about:'],
...
describe('generate_urls', () => {
  Object.entries(test_urls).forEach(([url, result]) => {
    it(`${url} — dumb domain detection`, () => {
      ...
      deepStrictEqual(generate_urls(url, true), result.map(strip_dumb_marker));
    });
    it(`${url} — smart domain detection`, () => {
      deepStrictEqual(
        smart_generate_urls(url),
        result.filter(
          (s) => s !== hint_marker && !s.startsWith(DUMB_DOMAIN_DETECTION),
        ),
      );
      deepStrictEqual(
...
const good_IPs = ['1.1.1.1', '12.34.56.78', '254.255.128.123', '0.0.0.0'];
